...
class YogaCorrector:
    ...
    def _cycle(self) -> None:
        ...
        if self._diag_counter % 25 == 0:
            ...
            logger.debug(
                "[ANGLES] raw=%-20s  confirmed=%-20s  %s",
                raw_pose.name, self._confirmed_inst_pose or 'none', angle_str
            )
    ...
    def _render(self, frame, user_pose: PoseResult) -> None:
        ...
        if not self.show_debug:
            # When running inside the web app, we skip native drawing.
            # We still need to process keyboard commands internally if they were pushed.
            return

        if not hasattr(self, "_mirror_window_created"):
            cv2.namedWindow("Yoga Mirror", cv2.WINDOW_NORMAL)
            cv2.setWindowProperty("Yoga Mirror", cv2.WND_PROP_TOPMOST, 1)
            self._mirror_window_created = True

        cv2.imshow("Yoga Mirror", vis)
