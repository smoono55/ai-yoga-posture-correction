...
# ── Process state ──────────────────────────────────────────────────────────────
_yoga_process: subprocess.Popen | None = None
...
LANGUAGES = {
    ...
    "kn": {"name": "Kannada",            "native": "🇮🇳", "tts_voice": "kn-IN-GeethaNeural",    "tts_rate": "+0%"},
...
def get_status():
    ...
    with _yoga_lock:
        running    = _yoga_process is not None and _yoga_process.poll() is None
        returncode = _yoga_process.poll() if _yoga_process else None
...
def start_yoga():
    ...
    with _yoga_lock:
        if _yoga_process and _yoga_process.poll() is None:
    ...
    def _run():
        ...
        # ── Step 3: Launch main.py — no terminal window ────────────────────────
        ...
        try:
            proc = _hidden_popen([sys.executable, str(MAIN_PY)], cwd=str(BASE_DIR))
            with _yoga_lock:
                _yoga_process = proc
            ...
            if proc.poll() is not None:
                # Process already died — read last lines of log for error
                error_tail = ""
                try:
                    lines = LOG_FILE.read_text(encoding="utf-8", errors="replace").strip().splitlines()
                    error_tail = " | ".join(lines[-4:]) if lines else "no output"
                except Exception:
                    pass
                _setup_status.update({
                    "running": False, "done": True, "error": True,
                    "message": f"main.py crashed (exit {proc.returncode}): {error_tail[:300]}",
                })
                return
            ...
            _setup_status.update({
                ...
                "message": f"Running in {lang_name} — webcam window should be open.",
            })
        except Exception as e:
            _setup_status.update({
                "running": False, "done": True, "error": True,
                "message": f"Failed to start main.py: {e}"
            })
...
def stop_yoga():
    ...
    with _yoga_lock:
        if _yoga_process and _yoga_process.poll() is None:
            _yoga_process.terminate()
            try:
                _yoga_process.wait(timeout=5)
