...
async def _edge_speak_async(text: str, voice=None, rate=None, volume=None) -> bool:
    ...
    try:
        ...
        if _IS_WINDOWS:
            import winsound
            # Convert mp3 → wav for winsound, or use pygame/playsound
            try:
                import playsound
                playsound.playsound(tmp, block=True)
            except ImportError:
                # Fall back to PowerShell media player
                subprocess.run(
                    ["powershell", "-c",
                     f'(New-Object Media.SoundPlayer).PlaySync(); '
                     f'$p=[System.Media.SoundPlayer]::new(); '
                     f'Add-Type -AssemblyName presentationCore; '
                     f'$mp=New-Object System.Windows.Media.MediaPlayer; '
                     f'$mp.Open([System.Uri]("{tmp}")); $mp.Play(); Start-Sleep 4'],
...
def _try_init_pyttsx3():
    try:
        ...
        for v in voices:
            ...
            if any(x in vid+vname for x in ["zira","jenny","female","samantha","hazel"]):
...
def _os_speak(text: str) -> None:
    ...
    try:
        if os_name == "Darwin":
            subprocess.run(["say", "-v", "Samantha", "-r", "170", text],
                           timeout=15, check=False)
        elif os_name == "Linux":
...
class AudioFeedback:
    ...
    @staticmethod
    def _say(message: str, pyttsx3_engine, edge_ok: bool) -> None:
        if edge_ok:
