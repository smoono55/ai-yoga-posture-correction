...
STRINGS: dict[str, str] = {
    ...
    "cam_too_far":
        "You're a little far. Please move closer so I can see your full body.",
    "cam_too_close":
        "You're too close! Please step back so your full body fits in the frame.",
    "cam_sideways":
        "It looks like you might be turned sideways. Please face the camera directly.",
...
# ── Pose steps (from pose_classifier.py) ─────────────────────────────────────
def _get_pose_steps() -> dict[str, str]:
    sys.path.insert(0, str(BASE))
    try:
        ...
        print(f"  Warning: Could not import pose steps: {e}")
        return {}

# ── Translator ────────────────────────────────────────────────────────────────
def _translate_all(texts: dict[str, str], target_lang: str) -> dict[str, str]:
    """
    Translate all strings using deep-translator (GoogleTranslator).
    Translates one string at a time — reliable and rate-limit safe.
...
def generate(lang: str, force: bool = False) -> None:
    ...
    all_strings.update(_get_pose_steps())
